...
jenkinsfileURL = "https://github.com/hernanex97/apic-test"
...
node('principal') { //This is the label which is defined in Jenkins Master and runs in worker node
    ...
    try{
        ...
        def environmentChoices = ['Dev', 'Test', 'Produccion'].join('\n')
...
def Deploy(String server, String creds, String product, String catalog, String org, String realm, 
    ...
    String productPlanMapFile = "") {
        ...
        if (versioning == "false") {
            ...
            def status = Publish(product, catalog, org, server)
            ...
            def status = Stage(stagingProduct, catalog, org, server)
            ...
            status = Replace(stagingProductname, newVersion, productPlanMapFile, catalog, org, server)
            if(status != 0) 
            {
                currentBuild.result = "FAILED"
...
def Stage(String product, String catalog, String org, String server) {
...
def Replace(String productName, String newVersion, String productPlanMapFile, String catalog, String org, String server){
    echo "Replacing the existing product ${productName} with the new version of the product ${newVersion}"
    def status = sh script: "/bin/apic products:replace --scope catalog ${productName}:${newVersion} ${productPlanMapFile} --catalog ${catalog} --org ${org} --server ${server}",
        returnStatus:true   
    return status
}
...
def ValidateCCAndDeploy(String server, String creds, String product, String catalog, String org, String realm,
    String leadDeveloperID, String appdevManagerID, String appdevManagerEmail, 
    String environment, String productName) {
        ...
        if (apiVersioning == 'No') {
            //Publish the product 
            Deploy(server, creds, product, catalog, org, realm)
        }
        else {  
            ...
            Deploy(server, creds, product, catalog, org, realm, "true", stagingProduct, stagingProductName, newVersion, productPlanMapFile)                
        }
